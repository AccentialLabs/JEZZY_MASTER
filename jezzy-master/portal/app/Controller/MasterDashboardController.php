...
class MasterDashboardController extends AppController{
    //put your code here
     ...
     public function index() {
		...
		$indicationsCount = $this->getIndications();
				...
				$arrayUsersActives = $this->listActiveUsersLastHour();
		...
		$this->set('qtdSaloes', $qtd);
				...
				$this->set('arrayUsersActives', $arrayUsersActives);
	...
	public function listActiveUsersLastHour(){
	
		 $sql = "select name,last_update,photo from users where last_update >= CURDATE() order by last_update desc;";
		   $activeUsers = array(
                        'User' => array(
                            'query' => $sql
                        )
